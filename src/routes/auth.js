...
// Helper functions (unchanged except for sendActivationEmail)
...
// Helper function to send activation email (fixed to use BACKEND_URL)
const sendActivationEmail = async (user, activationToken) => {
    const transporter = nodemailer.createTransport({
        host: process.env.GMAIL_HOST || "smtp.gmail.com",
        port: Number.parseInt(process.env.GMAIL_PORT) || 587,
        secure: process.env.GMAIL_SECURE === "true", // false for 587
        auth: {
            user: process.env.GMAIL_USER,
            pass: process.env.GMAIL_PASS,
        },
        tls: {
            rejectUnauthorized: false, // Consider removing in production for stricter TLS
        },
    });

    await transporter.verify();

    // Use BACKEND_URL for activation link
    const activationUrl = `${process.env.BACKEND_URL || "https://dern-backend-fbtx.onrender.com"
        }/api/v1/auth/activate/${activationToken}`;
    const mailOptions = {
        to: user.email,
        from: process.env.GMAIL_FROM || "Dern Support <noreply@dern-support.com>",
        subject: "Buyerdan Your Dern Support Account",
        html: `
...
// Register endpoint (unchanged, already uses sendActivationEmail)
...
// Activate endpoint (unchanged, already redirects to frontend)
...
// Resend activation email (updated to use BACKEND_URL)
...
router.put("/profile", authenticate, async (req, res) => {
    try {
        ...
        // Update user
        const updatedUser = await User.findByIdAndUpdate(req.user.userId, value, {
            new: true,
            runValidators: true,
...
// Admin-only route to create admin users (unchanged)
